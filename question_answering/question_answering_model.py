...
from question_answering.config.model_args import QuestionAnsweringArgs
from question_answering.question_answering_utils import (
...
# from question_answering.bert_kgqa_model import QAModel
from question_answering.qa_att_model_2_trane import QAModel
...
class QuestionAnsweringModel:
    def __init__(
        ...
        MODEL_CLASSES = {
            ...
            "bert": (BertConfig, QAModel, BertTokenizer),
            "camembert": (
                CamembertConfig,
                CamembertForQuestionAnswering,
                CamembertTokenizer,
            ...
            "roberta": (RobertaConfig, QAModel, RobertaTokenizer),
            "squeezebert": (
                SqueezeBertConfig,
                SqueezeBertForQuestionAnswering,
                SqueezeBertTokenizer,
        ...
        print("!!!!!!!!!  ############  ",model_class, model_name, model_type,"  ############  !!!!!!!!!")
        ...
        if not self.args.quantized_model:
            ...
            if model_type =='bert':
                ...
                relation_dic = pickle.load(open(rel_dic,'rb'))
                print("roberta relation_dic loaded in question_answering_model")
                n_voc = open(vocab_path).read().split('\n')[:-1]
                ...
                for i,item in enumerate(n_voc):
                    vocab_dic[item] = i
                self.model = model_class(model_name, self.config.hidden_size, len(vocab_dic), len(relation_dic), 300)
            else:

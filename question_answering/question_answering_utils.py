...
print("loading pharm kg file")
with open('../../../Data/pharm_kg.json','r') as f:
    pharmkg=json.load(f)

print("pharmkg loaded !!!!!!!!!!!!!!!!")


print("loading mashqa-umls file")
with open('../../../Data/mashqa_kg_1.json','r') as f:
    mashqakg=json.load(f)

print("mashqa_kg_1 loaded !!!!!!!!!!!!!!!!")

# kg_file="Data/raw_PharmKG-180k.csv"

# pharmkg = pd.read_csv(kg_file)
# print("pharmkg loaded !!!!!!!!!!!!!!!!")
...
# print("loading conceptnet kg")
# conceptnet_kg="Data/conceptnet_triplets.csv"
# cn_kg = pd.read_csv(conceptnet_kg)
# cn_kg= cn_kg.iloc[: , 1:]
# print("shape of conceptnet_kg: ",cn_kg.shape)

relation_list=['manifestation_of','uses','treats','prevents','physically_related_to','method_of','isa','diagnoses','co-occurs_with','carries_out','associated_with','affects','adjacent_to']

def return_kg(final_keyword,dec_word,pharmkg,mashqakg,filter=True):
    kg_triplet=[]
    tail=[]
    dec_words = set(dec_word)
    for word in final_keyword:
        if filter:
            if word in pharmkg.keys() and word not in nltk_stopwords and len(word) > 2:
                for d_word in dec_words:
                    for j, triples in enumerate(pharmkg[word]):
                        if d_word in triples[2]:
                            kg_triplet.append([word.replace(" ","_"),triples[1],triples[2]])
                            tail.append(triples[2])
            elif word in mashqakg.keys() and word not in nltk_stopwords and len(word) > 2:
                for d_word in dec_words:
                    for j, triples in enumerate(mashqakg[word]):
                        if d_word in triples[1] and triples[0] in relation_list:
                            kg_triplet.append([word.replace(" ","_"),triples[0],triples[1]])
    ...
    # for t in kg_triplet:
    #     tail.append(t[2].replace("_"," "))
    return kg_triplet, tail

def n_hops(text,dec_word):
    NAF = ["_NAF_H", '_NAF_R', "_NAF_T"]
    triples_len = 512
    triple=[]
    tail=[]
    #1st hops
    T1,t1 = return_kg(text,dec_word,pharmkg,mashqakg,filter=False)
    triple.extend(T1)
    tail.extend(t1)
    # if n==1:
    #     print("hop==1")
    ...
    # print(triple)
    # print(len(triple))
    T2,t2 = return_kg(t1,dec_word,pharmkg,mashqakg,filter=True)
    # print("hop==2")
    triple.extend(T2)
    tail.extend(t2)
    # print(tail)
...
# def convert_tokens_to_id(kg,vocab_dic,relation_dic):
#     kg = [[transform_triple_to_hrt(triple,vocab_dic,relation_dic)  for triple in seq_item] for seq_item in kg]
#     return kg
...
def squad_convert_example_to_features(example, tokenizer, max_seq_length, doc_stride, max_query_length, padding_strategy, is_training):
    ...
    # print(example)
    # print("!!!!!!!!!!!")
    # print(tokenizer, max_seq_length,doc_stride,max_query_length,padding_strategy,is_training)
    # print("relation \n",relation[:5])

    tokenizer_type = type(tokenizer).__name__.replace("Tokenizer", "").lower()
    # print("!!!!!!!! tokenizer_type: ",tokenizer_type)
    # tokenizer.save_vocabulary('./vocab/bert/')
    ...
    if tokenizer_type == 'roberta':
        ...
        vocab_path="vocab/roberta/pharmKG_vocab.txt"
    ...
    for span in spans:
        ...
        if tokenizer_type == 'bert' or 'roberta':            
            q= re.sub(r'[^\w\s]','',example.question_text)
            q_tokens=q.split()
            kg_enc=n_hops(q_tokens,all_doc_tokens)
            kg_enc_input = convert_tokens_to_id(kg_enc,vocab_dic,relation_dic)
            # print("!!!!!!!!!!!!!!!!! span['tokens'] length  !!!!!!!!!!!!")
            # print(span["tokens"])
            # print(len(span["tokens"]))
            # kg_tokens = span["tokens"][:]

            # while len(kg_tokens) < max_seq_length:
            #     kg_tokens.append("[PAD]")

            # print(len(kg_tokens))
            # assert len(kg_tokens) == max_seq_length

            # kg_enc = return_kg(kg_tokens,keys,values,relation)
            # kg_enc_input = convert_tokens_to_id(kg_enc,vocab_dic,relation_dic)
